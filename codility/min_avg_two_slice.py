...
(notice that the slice contains at least two elements). The average of a slice
(P, Q) is the sum of A[P] + A[P + 1] + ... + A[Q] divided by the length of the
slice. To be precise, the average equals (A[P] + A[P + 1] + ... + A[Q]) / (Q − P + 1).
...
def solution(arr):
    # Founded index slice
    found_ind = 0
    # Start and end slice position
    start = 0
    end = 2

    # Function for calculate average slice
    sl = lambda x,y: sum(arr[x:y])/float(y - x)

    # Varible for keep value min average
    min_avg = sl(start, end)

    while end < len(arr):
        # Checked slices with 2 and 3 length
        for x, y in enumerate((1, 0)):
            end += y
            start += x

            next_sl = sl(start, end)
